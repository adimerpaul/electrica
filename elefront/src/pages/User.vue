  <template>
    <div class="q-pa-md">
      ...
      <q-table :filter="filter" title="REGISTRO DE USUARIOS" :data="data" :columns="columns" row-key="name" :rows-per-page-options="[50,100]">
    ...
    methods: {
      ...
      cambiopass (i) {
        ...
        this.$q.dialog({
          ...
          this.$q.loading.show()
          this.$axios.put('updatePassword/'+i.row.id,{password:data}).then(res=>{
            this.$q.loading.hide()
          })
        }).onCancel(() => {
        }).onDismiss(() => {
